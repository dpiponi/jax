...
class custom_jvp:
  __slots__ = ["fun", "nondiff_argnums", "jvp", "_sig", "__weakref__"]
...
def _merge_thunks(f, g):
  try:
    ...
    try:
      ...
      raise lu.StoreException("both stores occupied") from None

class custom_vjp:
  __slots__ = ["fun", "nondiff_argnums", "fwd", "bwd", "_sig", "__weakref__"]
  ...
  def __call__(self, *args, **kwargs):
    ...
    flat_fwd, out_data = _flatten_fwd(lu.wrap_init(self.fwd), in_tree)
    flat_bwd = _flatten_bwd(lu.wrap_init(self.bwd), in_tree, out_data)
    out_flat = custom_vjp_call(flat_fun, *args_flat, fwd=flat_fwd, bwd=flat_bwd,
                               out_data=out_data)
    return tree_unflatten(_merge_thunks2(out_tree, out_data), out_flat)
...
def _flatten_fwd(in_tree, *args):
  ...
  py_outs, py_res = yield py_args, {}
  outs, out_tree = tree_flatten(py_outs)
  res, res_tree = tree_flatten(py_res)
  yield res + outs, (out_tree, res_tree, res_tree.num_leaves)

@lu.transformation
def _flatten_bwd(in_tree, out_data, *args):
  out_tree, res_tree, num_res = out_data()
  ...
  cts_in, _ = tree_flatten(py_cts_in)
  yield cts_in

def _merge_thunks2(f, g):
  try:
    out = f()
  except lu.StoreException:
    try:
...
# def custom_vjp(fwd, bwd, nondiff_argnums=()):
#   @wraps(fwd)
#   def fun_(*args, **kwargs):
#     if nondiff_argnums:
#       dyn_argnums = [i for i in range(len(args)) if i not in nondiff_argnums]
#       fwd_, dyn_args = _argnums_partial_placeholders(lu.wrap_init(fwd),
#                                                      dyn_argnums, args)
#     else:
#       fwd_, dyn_args = lu.wrap_init(fwd), args
#     args_flat, in_tree = tree_flatten((dyn_args, kwargs))
#     flat_fun, out_data = _flatten_fun_and_count_res(fwd_, in_tree)
...
def _partial_eval_custom_call(trace, call_primitive, fun, tracers, params):
  ...
  for bd_in in bdims:
    ...
    out_bdims.append(bd_out)
  outs = fun.call_wrapped(*tracers)
  for bd_out, s in zip(out_bdims, stores):
    s.store(bd_out())
  return outs
pe.JaxprTrace.process_custom_call = _partial_eval_custom_call

def _jvp_process_custom_call(trace, call_primitive, fun, tracers, params):
  in_bdims, stores = params.get('bdims', ()), params.get('stores', ())
  primals_in, tangents_in = unzip2((t.primal, t.tangent) for t in tracers)
  if call_primitive is custom_jvp_call_p:
    tangents_in = map(ad.instantiate_zeros, primals_in, tangents_in)
    jvpfun, out_bdims = params['jvp'], []
    for bd_in in in_bdims:
      jvpfun, bd_out = batching.batch_fun2(jvpfun, True, bd_in * 2)
      out_bdims.append(bd_out)
    outs = jvpfun.call_wrapped(*it.chain(primals_in, tangents_in))
    primals_out, tangents_out = split_list(outs, [len(outs) // 2])
    for bd_out, s in zip(out_bdims, stores):
      s.store(bd_out()[:len(outs) // 2])
    return map(partial(ad.JVPTracer, trace), primals_out, tangents_out)
  elif call_primitive is custom_vjp_call_p:
    fwd, out_bdims = params['fwd'], []
    for bd_in in in_bdims:
      fwd, bd_out = batching.batch_fun2(fwd, True, bd_in)
      out_bdims.append(bd_out)
    res_and_primals_out = fwd.call_wrapped(*map(core.full_lower, primals_in))
    out_tree, res_tree, num_res = params['out_data']()
    res, primals_out = split_list(res_and_primals_out, [num_res])
    res_and_out_bdims = []
    for bd_out, s, in_bdims_ in zip(out_bdims, stores, in_bdims):
      res_bdims_, out_bdims_ = split_list(bd_out(), [num_res])
      s.store(out_bdims_)
      res_and_out_bdims.append((res_bdims_, out_bdims_, in_bdims_))
    argnums, nz_tangents = unzip2((i, t) for i, t in enumerate(tangents_in)
                                  if t is not ad_util.zero)
    avals_out = [raise_to_shaped(core.get_aval(x)) for x in primals_out]
    tangents_out = custom_linearized_p.bind(
        *it.chain(res, nz_tangents), num_res=num_res, bwd=params['bwd'],
        avals_out=avals_out, out_tree=out_tree, res_tree=res_tree,
        bdims=res_and_out_bdims)
    return map(partial(ad.JVPTracer, trace), primals_out, tangents_out)
  else:
    raise ValueError(call_primitive)
ad.JVPTrace.process_custom_call = _jvp_process_custom_call
